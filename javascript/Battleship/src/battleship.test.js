...
test("carrier must equal {ship: { Carrrier: 4, Battleship: 4, Cruiser: 3, Submarine: 3, Destroyer: 2 }, status: true}", () => {
  expect(ship(myship).hit("Carrier")).toEqual({
    ship: { Carrier: 4, Battleship: 4, Cruiser: 3, Submarine: 3, Destroyer: 2 },
    status: true,
  });
});

test("battleship must equal {ship: { Carrrier: 5, Battleship: 3, Cruiser: 3, Submarine: 3, Destroyer: 2 }, status: true}", () => {
  expect(ship(myship).hit("Battleship")).toEqual({
    ship: { Carrier: 4, Battleship: 3, Cruiser: 3, Submarine: 3, Destroyer: 2 },
    status: true,
  });
});

test("destroyer must equal {ship: { Carrrier: 5, Battleship: 4, Cruiser: 3, Submarine: 3, Destroyer: 1 }, status: true})", () => {
  expect(ship(myship).hit("Destroyer")).toEqual({
    ship: { Carrier: 4, Battleship: 3, Cruiser: 3, Submarine: 3, Destroyer: 1 },
    status: true,
  });
});

test("submarine must equal {ship: { Carrrier: 5, Battleship: 4, Cruiser: 3, Submarine: 2, Destroyer: 2 }, status: true}", () => {
  expect(ship(myship).hit("Submarine")).toEqual({
    ship: { Carrier: 4, Battleship: 3, Cruiser: 3, Submarine: 2, Destroyer: 1 },
    status: true,
  });
});

test("cruiser must equal {ship: { Carrrier: 5, Battleship: 4, Cruiser: 2, Submarine: 3, Destroyer: 2 }, status: true}", () => {
  expect(ship(myship).hit("Cruiser")).toEqual({
    ship: { Carrier: 4, Battleship: 3, Cruiser: 2, Submarine: 2, Destroyer: 1 },
    status: true,
  });
});
...
test("Board must sunk", () => {
  expect(
    gameBoard().receiveAttack("E5", board2, {
      Carrier: 0,
      Battleship: 0,
      Cruiser: 0,
      Submarine: 0,
