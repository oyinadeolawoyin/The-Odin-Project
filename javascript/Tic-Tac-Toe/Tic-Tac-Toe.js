...
function PlayGame(numOfGrid) {
  ...
  const getComputerMove = () => {
    let randomCellsPicking =
      gridList[Math.floor(Math.random() * gridList.length)];
...
function GameController() {
    ...
    for (rows in board) {
      ...
      for (let i = 1; i < board[rows].length; i++) {
        ...
        if (getfistRowCell === getRows && i != board[0].length - 1) {
          ...
          if (
            (getfistRowCell === getRows &&
              i === board[rows].length - 1 &&
              getRows !== undefined) ||
            getfistRowCell !== undefined
          ) {
            return true;
          }
        }
  ...
  const checkingDraw = (board) => {
    for (element in board) {
      for (i = 0; i < board[element].length; i++) {
        if (
          board[element][i][0] !== undefined &&
          i !== board[element].length - 1
        ) {
          continue;
  ...
  const winningSearch = (board) => {
    if (
      checkingColumns(board) === true ||
      checkingRows(board) === true ||
      checkingLeftDiagonal(board) === true ||
      checkingRightDiagonal(board) === true
    ) {
      return true;
    } else {
          ...
          element.addEventListener("click", () => {
            ...
            if (index === 0) {
              ...
              if (checkingElement === 1) {
                ...
                let computerMoves = doc.querySelector(
                  `.number${getComputerMove[0]}`,
                );
              ...
              if (game === true) {
                ...
                message.appendChild(
                  doc.createTextNode(`${playerList[1]} Wins!`),
                );
