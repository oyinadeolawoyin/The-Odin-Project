function moves(row, col, path) {
    ...
    let upRight = [[row + 2, col + 1], [...path, ...[row + 2, col + 1]]];
    let upLeft = [[row + 2, col - 1], [...path, ...[row + 2, col - 1]]];
    let downRight = [[row - 2, col + 1], [...path, ...[row - 2, col + 1]]];
    let downLeft = [[row - 2, col - 1], [...path, ...[row - 2, col - 1]]]; 
    let leftUp = [[row + 1, col - 2], [...path, ...[row + 1, col - 2]]];
    let leftDown = [[row - 1, col - 2], [...path, ...[row - 1, col - 2]]];
    let rightUp = [[row + 1, col + 2], [...path, ...[row + 1, col + 2]]];
    let rightDown = [[row - 1, col + 2], [...path, ...[row - 1, col + 2]]];
...
function graph(array) {
    ...
    for (let i = 0; i < array.length; i++) {
        if (array[i][0][0] >= 8 || array[i][0][1] >= 8 || array[i][0][0] < 0 || array[i][0][1] < 0) continue;
...
function knightsMoves(start, target) {
    ...
    while (queue.length > 0) {
        ...
        if (currentNode[0][0] === target[0] && currentNode[0][1] === target[1]) {
            console.log(`you made it in ${currentNode[1].length/2} moves! Here is your paths:`)
            ...
            let move = graph(moves(currentNode[0][0], currentNode[0][1], currentNode[1]));
