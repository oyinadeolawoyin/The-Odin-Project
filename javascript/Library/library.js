...
function displayBook(infoBook) {
  ...
  let paraText1 = document.createTextNode(
    `Title: ${infoBook[infoBook.length - 3]}`,
  );
  let paraText2 = document.createTextNode(
    `Author: ${infoBook[infoBook.length - 2]}`,
  );
  let paratext3 = document.createTextNode(
    `Pages: ${infoBook[infoBook.length - 1]}`,
  );
...
removemyBook.forEach((button) => {
  button.addEventListener("click", (event) => {
    ...
    list.push(
      bookClass1.textContent,
      bookClass2.textContent,
      bookClass3.textContent,
    );
    for (item in list) {
      let word = list[item];
      let index = 0;
