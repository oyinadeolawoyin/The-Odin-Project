...
function operators(array) {
  ...
  if (array.operators === "+") {
    ...
    return cal.subract(
      parseInt(array.firstNumber),
      parseInt(array.secondNumber),
    );
  } else if (array.operators === "*") {
    return cal.muitply(
      parseInt(array.firstNumber),
...
buttons.forEach((button) => {
  button.addEventListener("click", () => {
    if (
      checkNextWord[0] === calculate &&
      listOfOperators.includes(button.textContent) &&
      count === 1
    ) {
...
function totalAnswer() {
  ...
  let bigDisplayText = document.createTextNode(
    `${numberArray2.join(" ")} = ${calculate}`,
  );
